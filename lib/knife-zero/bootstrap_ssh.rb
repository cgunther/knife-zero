    ...
    if Gem::Version.new(Chef::Knife::VERSION) < Gem::Version.new("17.3.27")
      def ssh_command(command, subsession = nil) # rubocop:disable Metrics/PerceivedComplexity, Metrics/AbcSize, Metrics/CyclomaticComplexity
        if config[:client_version]
          ...
          when 'cinc'
            super(%{/opt/cinc/embedded/bin/ruby -ropen-uri -e 'puts URI.open("https://omnitruck.cinc.sh/install.sh").read' | sudo sh -s -- -v #{config[:client_version]}})
          else
            super(%{/opt/chef/embedded/bin/ruby -ropen-uri -e 'puts URI.open("https://omnitruck.chef.io/chef/install.sh").read' | sudo sh -s -- -v #{config[:client_version]}})
          end
        end
